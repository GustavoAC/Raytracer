...
void Background::setTexture(const std::string _texturePath, int repeat_horizontal_size,
...
Pixel Background::getColor(float i, float j) {
    ...
    Pixel currentBottom(bottomLeft.getR() + ((bottomRight.getR() - bottomLeft.getR()) * i),
                        bottomLeft.getG() + ((bottomRight.getG() - bottomLeft.getG()) * i),
                        bottomLeft.getB() + ((bottomRight.getB() - bottomLeft.getB()) * i));

    Pixel current(currentTop.getR() + ((currentBottom.getR() - currentTop.getR()) * j),
...
const Pixel& Background::getBottomLeft() const { return m_colors[m_numColors - 1]; }
